...
class CommonFunc {
 ...
 public:
  ...
  typedef rrl::Tensor<Dtype, 3> StateTensor;
  typedef rrl::Tensor<Dtype, 3> ActionTensor;
  ...
  using ReplayMemory_ = rrl::Memory::ReplayMemorySARS<Dtype, StateDim, ActionDim>;
  ...
  static Result runEpisode(Task_ *&task,
    ...
    rai::TerminationType termType = rai::TerminationType::not_terminated;
    State state_tp, state_tp1;
    rrl::Tensor<Dtype,3> state({StateDim, 1, 1}, "state");
    rrl::Tensor<Dtype,3> action({ActionDim, 1, 1}, "sampledAction");
    ...
    if (task->isTerminalState()) {
      ...
      trajectory.terminateTrajectoryAndUpdateValueTraj(
          rai::TerminationType::terminalState, state_tp, action_tp,
          task->termValue(), task->discountFtr());
    }

    while (termType == rai::TerminationType::not_terminated) {
      ...
      if (task->dt() * stepCount + task->dt() * 0.5 >= timeLimit)
        termType = rai::TerminationType::timeout;

      if (termType == rai::TerminationType::terminalState) {
        trajectory.terminateTrajectoryAndUpdateValueTraj(
            termType, state_tp1, action_tp,
            task->termValue(), task->discountFtr());
      } else if (termType == rai::TerminationType::timeout) {
        trajectory.terminateTrajectoryAndUpdateValueTraj(
...
//  }
  ...
  static Result runEpisodeInBatchParallel(std::vector<Task_ *> &taskset,
    ...
    rai::TerminationType termtype_t[ThreadN];
    Dtype cost[ThreadN];
    int trajcnt = 0;
        ...
        while (isTerminal || isTimeout) {
          ...
          if (isTerminal) {
            trajectorySet[trajectoryID[i]].terminateTrajectoryAndUpdateValueTraj(
                rai::TerminationType::terminalState, state_t[i], action_t[i],
                taskset[0]->termValue(), taskset[i]->discountFtr());
          } else if (isTimeout) {
            trajectorySet[trajectoryID[i]].terminateTrajectoryAndUpdateValueTraj(
                rai::TerminationType::timeout, state_t[i], action_t[i],
...
#pragma omp parallel for schedule(dynamic) reduction(+:stepCount)
  ...
  static void takeOneStep(std::vector<Task_ *> &task,
    ...
    State state_tp1;
    Dtype cost;
    rai::TerminationType termType_tp1;

    termType_tp1 = rai::TerminationType::not_terminated;
    ...
    if (termType_tp1 != rai::TerminationType::not_terminated) {
  ...
  static void takeOneStepInBatch(std::vector<Task_ *> &tasks,
    ...
    rai::TerminationType termType_tp1[threadN];

    for (int i = 0; i < threadN; i++) {
      termType_tp1[i] = rai::TerminationType::not_terminated;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < threadN; i++) {
      ...
      if (termType_tp1[i] != rai::TerminationType::not_terminated) {
