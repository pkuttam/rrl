...
class TrajectoryAcquisitor : public Acquisitor<Dtype, StateDim, ActionDim> {
  ...
  using DataSet = rrl::Algorithm::LearningData<Dtype, StateDim, ActionDim>;
 ...
 public:
  ...
  void acquireNEpisodes(std::vector<Task_ *> &task,
    ...
    rai::Utils::timer->startTimer("Simulation");
    double dt = task[0]->dt();
    ...
    rai::Utils::timer->stopTimer("Simulation");
  }
  ...
  void acquireVineTrajForNTimeSteps(std::vector<Task_ *> &task,
    ...
    rrl::Op::VectorHelper::sampleRandomStates(traj, VineStartPosition, int(0.1 * timeLimit / dt), indx);
