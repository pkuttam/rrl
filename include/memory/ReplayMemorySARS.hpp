 ...
 public:
  ...
  inline void sampleRandomBatch(ReplayMemorySARS &batchMemory) {
    ...
    for (unsigned i = 0; i < batchSize; i++) {
      ...
      rai::TerminationType termination = rai::TerminationType(terminationFlag_->at(memoryIdx[i]));
...
template<typename Dtype, int stateDimension, int actionDimension>
std::mutex rrl::Memory::ReplayMemorySARS<Dtype, stateDimension, actionDimension>::memoryMutex_;
