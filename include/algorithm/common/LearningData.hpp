...
class LearningData {
  ...
  typedef rrl::Tensor<Dtype, 1> Tensor1D;
  typedef rrl::Tensor<Dtype, 2> Tensor2D;
  typedef rrl::Tensor<Dtype, 3> Tensor3D;
...
//    stdevs = "stdevs";
  ...
  void appendTrajs(std::vector<Trajectory_> &traj, Task_ *task, bool isRecurrent_ = false, ValueFunc_ *vf = nullptr) {
    ...
    if (vf) {
      ...
      for (int traID = 0; traID < traj.size(); traID++)
        if (traj[traID].termType == rai::TerminationType::timeout) {
...
//    Utils::timer->stopTimer("fillminiBatch");
  ...
  void divideSequences(int segLen, int stride = 1, bool keepHiddenState = true) {
    ...
    std::vector<rrl::Tensor<Dtype, 3>> extraTensor3D_t;
    std::vector<rrl::Tensor<Dtype, 2>> extraTensor2D_t;
    std::vector<rrl::Tensor<Dtype, 1>> extraTensor1D_t;
...
//          hiddenStates.batch(segID) =  hiddenStates_t.batch(segID).block(0,stride*j,hDim,segLen);
  ...
  void computeAdvantage(std::vector<Trajectory_> &traj,
    ...
    rai::Utils::timer->startTimer("GAE");
    for (auto &tra : traj) {
 ...
 public:
  ...
  //vectors for additional data
  std::vector<rrl::Tensor<Dtype, 3>> extraTensor3D;
  std::vector<rrl::Tensor<Dtype, 2>> extraTensor2D;
  std::vector<rrl::Tensor<Dtype, 1>> extraTensor1D;
