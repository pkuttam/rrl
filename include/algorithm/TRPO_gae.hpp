...
//  EIGEN_MAKE_ALIGNED_OPERATOR_NEW
  ...
  typedef rrl::Algorithm::LearningData<Dtype, StateDim, ActionDim> Dataset;
 ...
 private:
  ...
  void VFupdate() {
    ...
    mixfrac = 0.1;
    rai::Utils::timer->startTimer("Vfunction update");
    ...
    rai::Utils::timer->stopTimer("Vfunction update");

    LOG(INFO) << "value function loss : " << loss;
  }

  void TRPOUpdater() {
    rai::Utils::timer->startTimer("policy Training");
    ...
    rai::Utils::timer->startTimer("Gradient computation");
    policy_->TRPOpg(Dataset_, stdev_o, policy_grad);
    rai::Utils::timer->stopTimer("Gradient computation");
    ...
    rai::Utils::timer->startTimer("lineSearch");
    parameter_ += line_search(fullstep, expected);
    rai::Utils::timer->stopTimer("lineSearch");

    policy_->setLP(parameter_);
    updatePolicyVar();/// save stdev & Update Noise Covariance
    rai::Utils::timer->stopTimer("policy Training");
  }
