...
constexpr int sampleN = 36; // hight scan around each foot
...
class blind_locomotion {
 public:
  ...
  void updateHistory() {
    ...
    Eigen::Matrix<double, 3, 3> R_b = rai::Math::MathFunc::quatToRotMat(quat);

    Eigen::Matrix<double, 4, 1> quat2;
    quat2 = rai::Math::MathFunc::boxplusI_Frame(quat, e_g_bias_);
    Eigen::Matrix<double, 3, 3> R_b2 = rai::Math::MathFunc::quatToRotMat(quat2);
  ...
  virtual void integrate() {
    ...
    // joint history adjust (pos and vel ) using temp variable 
    ...
    // end of joint histroy adjust
      Eigen::Matrix<double, 6, 1> seaInput;
      Eigen::Matrix<double, 8, 1> seaInput2;

      // Torque at each joint is function of joint pos rediual and joint vel at t0,t-1,t-2
  ...
  void updateAction(const Action &action_t) {
    ...
    previousAction_ = scaledAction_.template cast<float>();
...
//            heights_[idx] -= pixelSize_;
  ...
  void init() {
    ...
    if (terrainType_ != TerrainType::Flat_) {
      ...
      quat = rai::Math::MathFunc::rotMatToQuat(R_b_);
      ...
      q_.template segment<4>(3) = rai::Math::MathFunc::rotateQuatByAngleAxis(quat, angle, terrainNormal_);
    } else {
      Eigen::Vector3d heading;
    ...
    quat2 = rai::Math::MathFunc::boxplusI_Frame(quat, e_g_bias_);
    R_b_noisy = rai::Math::MathFunc::quatToRotMat(quat2);
...
///terrain
  ...
  rrl::FuncApprox::MLP_fullyconnected<double, 6, 1, rrl::FuncApprox::ActivationType::softsign> actuator_A_;
