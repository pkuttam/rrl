...
#include <function/common/SimpleMLPLayer.hpp>
//#include "jhUtil.hpp"
...
class ANYmal_minimal : public Task<Dtype, StateDim, ActionDim, CommandDim> {
 ...
 public:
  ...
  explicit ANYmal_minimal(bool visualize = false,
    ...
    viswatch_.start();
    if (vis_on_) {
      env_.reset(new raisim::World(600, 600, 0.12f, rai_sim::NO_BACKGROUND));
...
//	task specific implementations
  inline void integrateOneTimeStep() {
    ...
    R_b_ = rrl::Math::MathFunc::quatToRotMat(quat);
...
//  *	 	 	 	 	 	   leg2- ... leg3- ... leg4- ...]
  ...
  bool isTerminalState(const VectorXd &q_term, const VectorXd &u_term) {
    ...
    double r, p, y;
    rrl::Math::MathFunc::QuattoEuler(quat, r, p, y);
    RotationMatrix R = rrl::Math::MathFunc::quatToRotMat(quat);
...
*       learning state =
  ...
  inline void conversion_LearningState2GeneralizedState(const State &state,
    ...
    Quaternion quat = rrl::Math::MathFunc::rotMatToQuat(R_b);
...
*       learning state =
  ...
  inline void conversion_GeneralizedState2LearningState(State &state,
    ...
    RotationMatrix R_b = rrl::Math::MathFunc::quatToRotMat(quat);
...
//  rai_sim::HeightMapHandle terrain_;
  ...
  rrl::FuncApprox::MLP_fullyconnected<double, 6, 1, rrl::FuncApprox::ActivationType::softsign> actuator_;
